...
{
    public function avatar(int $page, string $file)
    {
    ...
    public function media(int $page, int $post, string $file)
    {
        $path = 'pages/' . $page . '/' . $post . '/' . $file;
        if (Storage::disk('gcs')->exists($path)) {
            return Storage::disk('gcs')->download($path);
        }
    ...
    {
        echo '<h3>Política de Privacidade</h3>
                ...
                <p>Se após você nos fornecer seus dados, você mudar de ideia, você pode retirar o seu consentimento para que possamos entrar em contato, para a coleção de dados contínua, uso ou divulgação de suas informações, a qualquer momento, entrando em contato conosco em <b>renatomendes.apps@gmail.com</b> ou nos enviando uma correspondência em: <b>RENATO PEREIRA MENDES TECNOLOGIA DA INFORMACAO ME</b> <b>Rua Pirambóia, 67. Ap. 11 - Vila Carrão - São Paulo / SP</b></p>
